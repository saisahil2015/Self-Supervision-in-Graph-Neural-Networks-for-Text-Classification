    ...
    with open(origin_file, 'rb') as fopen:
        ...
        for sentence in root:
            ...
            for c in terms:
                ...
                print("Printing left word offset and right word offset")
                print(left_word_offset)
                print(to_word_offset)
...
def main():
    ...
    for train_file, test_file in data:
        # xml -> txt
        xml2txt(os.path.join(data_path, train_file))
        xml2txt(os.path.join(data_path, test_file))

        # txt -> json
        train_sentences = get_dependencies(
            os.path.join(data_path, train_file.replace('.xml', '_text.txt')), predictor)
        test_sentences = get_dependencies(os.path.join(
            data_path, test_file.replace('.xml', '_text.txt')), predictor)

        print(len(train_sentences), len(test_sentences))

        syntaxInfo2json(train_sentences, os.path.join(data_path, train_file))
        syntaxInfo2json(test_sentences, os.path.join(data_path, test_file))
